...
void DX::DeviceResources::CreateDeviceResources()
{
	...
	//create 2 types of rasterState. back cull first
	D3D11_RASTERIZER_DESC RasterizerStateDesc;
	ZeroMemory(&RasterizerStateDesc, sizeof(D3D11_RASTERIZER_DESC));
	RasterizerStateDesc.FillMode = D3D11_FILL_SOLID;
	RasterizerStateDesc.CullMode = D3D11_CULL_BACK;
	DX::ThrowIfFailed(device->CreateRasterizerState(&RasterizerStateDesc, &frontRasterState));
	RasterizerStateDesc.CullMode = D3D11_CULL_FRONT;
	DX::ThrowIfFailed(device->CreateRasterizerState(&RasterizerStateDesc, &backRasterState));

	//create second depth stencil state (first is nullptr)
	...
	this->m_d3dDevice->CreateDepthStencilState(&dsDesc, depthStencilState.ReleaseAndGetAddressOf());
}
...
void DX::DeviceResources::Present(CurrentSwapChain currentSwapChain)
{
	switch (currentSwapChain)
	{
		...
		{	
			...
			auto warp = swapChainWarpHandler.getWarp("SwapChain2DTransferFunction");
		
			DXGI_PRESENT_PARAMETERS parameters = { 0 };
		
			HRESULT hr = warp->swapChain->Present1(1, 0, &parameters);
			...
			m_d3dContext->DiscardView1(swapChainWarpHandler.getWarp("SwapChain2DTransferFunction")->renderTargetView.Get(), nullptr, 0);
...
void DX::DeviceResources::CatchSwapChainPanel(SwapChainPanel^ panel, string name) {

	DisplayInformation^ currentDisplayInformation = DisplayInformation::GetForCurrentView();
	std::shared_ptr<DX::SwapChainPanelWarp> p = this->swapChainWarpHandler.getWarp(name);
 
	p->swapChainPanel = panel;
	/*// Очистка контекста, связанного с размером предыдущего окна.
	ID3D11RenderTargetView* nullViews[] = { nullptr };
	m_d3dContext->OMSetRenderTargets(ARRAYSIZE(nullViews), nullViews, nullptr);
	m_d3dRenderTargetView = nullptr;
	...
	//UpdateRenderTargetSize(); они одного размера
	...
	*/
	...
	else*/
	{
		...
		m_swapChainPanel->Dispatcher->RunAsync(CoreDispatcherPriority::High, ref new DispatchedHandler([=]()
		{
			...
			DX::ThrowIfFailed(
				panelNative->SetSwapChain(p->swapChain.Get())
			);
		}, CallbackContext::Any));
	...
	DXGI_MODE_ROTATION displayRotation = ComputeDisplayRotation();
	DX::ThrowIfFailed(
		p->swapChain->SetRotation(displayRotation)
	);
	...
	ComPtr<IDXGISwapChain2> spSwapChain2;
	DX::ThrowIfFailed(
		p->swapChain.As<IDXGISwapChain2>(&spSwapChain2)
	);
	...
	ComPtr<ID3D11Texture2D1> backBuffer;
	DX::ThrowIfFailed(
		p->swapChain->GetBuffer(0, IID_PPV_ARGS(&backBuffer))
	);
	...
	DX::ThrowIfFailed(
		m_d3dDevice->CreateRenderTargetView(
			backBuffer.Get(),
			nullptr,
			p->renderTargetView.ReleaseAndGetAddressOf()
		)
	);
	...
	DX::ThrowIfFailed(
		m_d3dDevice->CreateDepthStencilView(
			depthStencil.Get(),
			&depthStencilViewDesc,
			p->d3dDepthStencilView.ReleaseAndGetAddressOf()
		)
	);
...
std::shared_ptr<DX::SwapChainPanelWarp> DX::SwapChainWarpHandler::getWarp(std::string name) {
	for (auto it = objects.begin(); it != objects.end(); ++it) {
		if ((*it)->name == name) return (*it);
	}
	this->objects.push_back(std::shared_ptr<DX::SwapChainPanelWarp>(new DX::SwapChainPanelWarp(name)));
	return this->getWarp(name);
}

DX::SwapChainWarpHandler* DX::DeviceResources::getSwapChainWarpHandler() {
	return &(this->swapChainWarpHandler);
}